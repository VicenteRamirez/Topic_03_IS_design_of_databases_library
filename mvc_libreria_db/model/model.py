...
class Model:
    ...
    *********************
    """
    def create_user(self, u_nombre, u_apellidopat, u_apellidomat, u_tel, correo, u_id_dir,u_cp):
        try:
            sql = 'INSERT INTO usuario (`u_nombre`, `u_apellidopat`, `u_apellidomat`, `u_tel`, `correo`, `u_id_dir`,`u_cp`) VALUES (%s,%s, %s, %s, %s, %s, %s)'
            vals = ( u_nombre, u_apellidopat, u_apellidomat, u_tel, correo, u_id_dir, u_cp )
            self.cursor.execute(sql, vals)
            self.cnx.commit()
            return True
        except connector.Error as err:
    ...
    def read_user(self, id_usuario):
        try:
            ...
            sql = 'SELECT usuario.*,direccion.d_calle,direccion.d_col,direccion.d_numExt FROM usuario JOIN direccion ON usuario.u_id_dir = direccion.id_dir and usuario.id_usuario = %s'
            #sql = 'SELECT usuario.*direccion.d_calle,direccion.d_col,direccion.d_numExt,cps.ciudad,cps.estado FROM usuario INNER JOIN direccion ON usuario.u_id_dir = direccion.id_dir INNER JOIN cps ON usuario.u_cp = cps.cp and usuario.id_usuario = %s'
            vals = (id_usuario,)
            self.cursor.execute(sql, vals)
            record = self.cursor.fetchone()
            return record
    ...
    def read_all_users(self):
        try:
            ...
            sql = 'SELECT usuario.*,direccion.d_calle,direccion.d_col,direccion.d_numExt FROM usuario JOIN direccion ON usuario.u_id_dir = direccion.id_dir'
            self.cursor.execute(sql)
            records = self.cursor.fetchall()
            return records
        except connector.Error as err:
    ...
    def read_user_ap(self, u_apellidopat):
        try:
            ...
            sql = 'SELECT usuario.*,direccion.d_calle,direccion.d_col,direccion.d_numExt FROM usuario JOIN direccion ON usuario.u_id_dir = direccion.id_dir and usuario.u_apellidopat = %s'
            vals = (u_apellidopat,)
            self.cursor.execute(sql,vals)
            records = self.cursor.fetchall()
            return records
    ...
    *********************
    ...
    def read_author_ap(self, a_apellidoPat):
        try:
            sql = 'SELECT * FROM autor WHERE a_apellidoPat = %s'
    ...
    ***********************
    ...
    def create_prestamo(self, pF_prestamo, pF_devolu,p_adeudo,p_id_usuario):
        try:
            ...
            self.cursor.execute(sql, vals)
            self.cnx.commit()
            id_prestamo = self.cursor.lastrowid
            return id_prestamo
        except connector.Error as err:
            self.cnx.rollback()
            return err
    ...
    def read_prestamo(self, id_prestamo):
        try:
            ...
            sql = 'SELECT prestamo.*,usuario.u_nombre,usuario.u_apellidopat FROM prestamo JOIN usuario ON prestamo.p_id_usuario = usuario.id_usuario and prestamo.id_prestamo = %s'
    ...
    def read_all_prestamos(self):
        try:
            ...
            sql = 'SELECT prestamo.*,usuario.u_nombre,usuario.u_apellidopat FROM prestamo JOIN usuario ON prestamo.p_id_usuario = usuario.id_usuario'
            self.cursor.execute(sql)
            records = self.cursor.fetchall()
            return records
        except connector.Error as err:
    ...
    def read_prestamo_user(self, p_id_usuario):
        try:
            ...
            sql = 'SELECT prestamo.*,usuario.u_nombre,usuario.u_apellidopat FROM prestamo JOIN usuario ON prestamo.p_id_usuario = usuario.id_usuario and prestamo.p_id_usuario = %s'
            vals = (p_id_usuario,)
            self.cursor.execute(sql,vals)
            records = self.cursor.fetchall()
            return records
    ...
    *********************************
    ...
    def read_detallesprestamo(self, d_id_prestamo):
        try:
            ...
            sql = 'SELECT detallesprestamo.*,libro.l_nombre,libro.l_edicion FROM detallesprestamo JOIN libro ON detallesprestamo.d_id_libro = libro.id_libro and detallesprestamo.d_id_prestamo = %s'
            vals = (d_id_prestamo,)
            self.cursor.execute(sql, vals)
            record = self.cursor.fetchone()
            return record
    ...
    def read_all_detallesprestamo(self):
        try:
            ...
            sql = 'SELECT detallesprestamo.*,libro.l_nombre FROM detallesprestamo JOIN libro ON detallesprestamo.d_id_libro = libro.id_libro'
            self.cursor.execute(sql)
            records = self.cursor.fetchall()
            return records
        except connector.Error as err:
    ...
    def read_detalleprestamo_book(self, d_id_libro):
        try:
            ...
            sql = 'SELECT detallesprestamo.*,libro.l_nombre FROM detallesprestamo JOIN libro ON detallesprestamo.d_id_libro = libro.id_libro and detallesprestamo.d_id_libro = %s'
            vals = (d_id_libro,)
            self.cursor.execute(sql,vals)
            records = self.cursor.fetchall()
            return records
